...
$sql = "SELECT * FROM `".$config['db']['pre']."user` WHERE user_type = 'freelancer'";
$sqlTot .= $sql;
$sqlRec .= $sql;
...
foreach ($queryRecords as $row) {
    ...
    $name = $row['name'];
    $type = $row['user_type'] == 'freelancer' ? 'Freelancer': 'Employer';
    $email = strlimiter($row['email'],18)."...";
    $category = strlimiter($row[category],40)."...";
    //$email = $row['email'];
    //$sex = $row['sex'];
    $image = $row['image'];
    $status = $row['status'];
    ...
    elseif($status == "1")
    {
        $status = '<img src="../assets/images/verified.png" style="max-width:60px;">';
    }
    else{
    ...
    $row4 = '<td class="hidden-xs hidden-sm">'.$category.'</td>';
